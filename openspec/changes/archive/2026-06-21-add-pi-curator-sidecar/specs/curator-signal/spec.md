...
## Purpose
The curator (separate pi process) signals findings back to its spawning main
session. ONE multiplex tool (`signal_main`) exposes two kinds; the curator LLM
picks per finding. Transport is the existing `pi-intercom` broker — no new IPC.
...
- Curator→curator cross-check signaling (deferred).
- Synchronous request/reply between curator and main (curator signals are
  fire-and-forget; main does not ack).
- Curator reading its own past signals (the curator is forked fresh each
  spawn; it has no memory of prior signals).
